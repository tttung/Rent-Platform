...
#####################################binary-classDNN###########################################
    def modeling(self):
        ###################################Data prepared#######################################
        '词嵌入'
        word_user_feature = np.array(cPickle.load( open(self.root + 'persistence/binary_user_word.pkl','rb')))  #用户序列内容嵌入
        word_news_feature = np.array(cPickle.load( open(self.root + 'persistence/binary_news_word.pkl','rb')))  #新闻词嵌入
        target = np.array(cPickle.load( open(self.root + 'persistence/binary_label.pkl','rb')))                 #label
        'deepWalk嵌入'
        graph_user_feature = np.array(cPickle.load( open(self.root + 'persistence/binary_user_graph.pkl','rb')))    #用户序列图嵌入
        graph_news_feature = np.array(cPickle.load( open(self.root + 'persistence/binary_news_graph.pkl','rb')))    #新闻图嵌入
        '词、图嵌入合并'
    ...
    def prepared(self):
        ######################recommend####################
        '词嵌入'
        word_user_feature = cPickle.load( open(self.root + 'persistence/multi_user_word.pkl','rb') )     #用户画像、时间上下文
        word_news_feature = cPickle.load( open(self.root + 'persistence/multi_news_word.pkl','rb') )     #新闻矩阵
        'deepWalk嵌入'
        graph_user_feature = cPickle.load( open(self.root + 'persistence/multi_user_graph.pkl','rb') )   #正负样本
        graph_news_feature = cPickle.load( open(self.root + 'persistence/multi_news_graph.pkl','rb') )

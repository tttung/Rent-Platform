    ...
    def split_dataset(self):
        ...
        for line in f1.readlines():
            ...
            if random.random() < pivot:
                ...
                testset_len += 1
        #用户-新闻阅读记录持久化
        cPickle.dump(self.trainset, open(self.root+'persistence/item_user_recrods.pkl', 'wb'))
...
#                    print('calculating movie similarity factor(%d)' % simfactor_count, file=sys.stderr)
        # item相似度矩阵持久化
        cPickle.dump(self.news_sim_mat, open(self.root+'persistence/item_sim_mat.pkl', 'wb'))
...
#        print('Total similarity factor number = %d' % simfactor_count, file=sys.stderr)
    ...
    '导入用户阅读新闻记录和新闻相似度矩阵'
    def prepared(self):
        self.trainset = cPickle.load( open(self.root + 'persistence/item_user_recrods.pkl','rb') ) #用户u ：用户已看过的item：评分
        self.news_sim_mat = cPickle.load( open(self.root + 'persistence/item_sim_mat.pkl','rb') )  #新闻m1： 新闻m2： 相似度

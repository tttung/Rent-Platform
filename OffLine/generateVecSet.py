...
class GenerateVecSet(object):
    def __init__(self):
        ...
        self.word_model = word2vec.Word2Vec.load('/Users/tung/Documents/Git/Rent-Platform/wordEmbedding/sougouCS_wordVec')                #加载已训练好的word2vec模型
        self.vocab = self.word_model.wv.vocab.keys()    # 先拿到全部的vocabulary

    def split_dataset(self):
        #导入行为数据,生成正样本
        f1=open(self.root+'dataset/train_date_set1.txt')
        ...
        # 新闻title持久化
        cPickle.dump(self.newsSet, open(self.root+'persistence/newsSet_title.pkl', 'wb'))
        ...
        #创建正负样本文件
        f=open(self.root+'dataset/dataset_Vec.csv','w')
    ...
    def get_stop_words(self):
        path = "/Users/tung/Documents/Git/Rent-Platform/wordEmbedding/chineseStopWords.txt"
    ...
    def news_embedding(self):
        ...
        cPickle.dump(newsSet2Vec, open(self.root+'persistence/multi_news_word.pkl', 'wb'))
    ...
    def user_embedding(self):
        ...
        cPickle.dump(user_profile, open(self.root + 'persistence/multi_user_word.pkl', 'wb'))
    ...
    def Generate_Set(self):
        ...
        cPickle.dump(user_lable, open(self.root + 'persistence/multi_user_lable.pkl', 'wb'))
        ...
        cPickle.dump(news_feature, open(self.root + 'persistence/binary_news_word.pkl', 'wb'))   #新闻embedding字典
        ...
        cPickle.dump(user_feature, open(self.root + 'persistence/binary_user_word.pkl', 'wb'))   #对应的embedding矩阵

        #持久化binary_lable
        df=pd.read_csv(self.root+'dataset/dataset_Vec.csv')
        cPickle.dump(df.label, open(self.root + 'persistence/binary_label.pkl', 'wb'))   #对应的embedding矩阵

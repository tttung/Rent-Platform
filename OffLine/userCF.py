...
#        print ('recommended news number = %d' % self.n_rec_news, file=sys.stderr)
    ...
    def split_dataset(self):
        ...
        #用户-新闻阅读记录持久化
        cPickle.dump(self.trainset, open(self.root+'persistence/user_news_recrods.pkl', 'wb'))
...
#                    print ('calculating user similarity factor(%d)' % simfactor_count, file=sys.stderr)
        # 用户相似度矩阵持久化
        cPickle.dump(self.user_sim_mat, open(self.root+'persistence/user_sim_mat.pkl', 'wb'))
                
    '导入用户阅读新闻记录和用户相似度矩阵'
    def prepared(self):
        self.trainset = cPickle.load( open(self.root + 'persistence/user_news_recrods.pkl','rb') ) #用户u ：用户已看过的item：评分
        self.user_sim_mat = cPickle.load( open(self.root + 'persistence/user_sim_mat.pkl','rb') )  #用户u ：相关用户v ：相似度

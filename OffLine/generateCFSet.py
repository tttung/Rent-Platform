...
############################划分正负样本#################################
    def split_dataset(self):
        ...
        #创建文件
        f=open(self.root+'dataset/dataset_CF.csv','w')
...
#############################提取特征#################################
    ...
    def insert_userCFScore(self):
        ...
        df=pd.read_csv(self.root+'dataset/dataset_CF.csv')

        df['userCFScore'] = userCFScore     #将新列的名字设置为userCFScore
        df.to_csv(self.root+'dataset/dataset_CF.csv', index = None)
...
#############################划分训练集与测试集#################################
    def Generate_Set(self):
        ...
        f2=open(self.root+'dataset/dataset_CF.csv')
        f3 = open(self.root+'dataset/trainset_CF.csv','w+')
        f3.write(",".join(["user_id", "new_id", "label", "userCFScore", "itemCFScore", "popular"]) + "\n")

        f4 = open(self.root+'dataset/testset_CF.csv','w+')

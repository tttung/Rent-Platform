...
class deepWalk(object):
    def __init__(self):
        ...
        self.dataDf=pd.read_csv(self.root+'dataset/dataset_CF.csv')
    ...
    def word2vec_embedding(self):
        ...
        cPickle.dump(self.newsSet, open(self.root + 'persistence/multi_news_graph.pkl', 'wb'))

        # user embedding矩阵
        for user, num in self.userSet.items():
            self.userSet[user] = model[user]

        cPickle.dump(self.userSet, open(self.root + 'persistence/multi_user_graph.pkl', 'wb'))
        ...
        cPickle.dump(user_feature, open(self.root + 'persistence/binary_user_graph.pkl', 'wb'))
        ...
        cPickle.dump(news_feature, open(self.root + 'persistence/binary_news_graph.pkl', 'wb'))
